...
public class RemoteStorage {
    ...
    private final String SHARE_PREFER_USER_INFO = "share_prefer_user_info";
    ...
    public void listBucket(String region, CosXmlResultListener resultListener) {
        ...
        if(!checkUserInfo(resultListener)) {
            return;
        }
    ...
    public boolean shouldInputUserInfo() {

        return TextUtils.isEmpty(APPID) || TextUtils.isEmpty(TEMP_HOST);
    }

    public void setUserInfo(String appid, String host, int port) {

        APPID = appid;
        TEMP_HOST = host;
        TEMP_PORT = port;

        SharedPreferences sharedPreferences = context.getSharedPreferences(SHARE_PREFER_USER_INFO, context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sharedPreferences.edit();
        editor.putString("appid", APPID);
        editor.putString("host", TEMP_HOST);
        editor.putInt("port", TEMP_PORT);
        editor.apply();
    }
    ...
    private void start() {
        ...
        TEMP_HOST = sharedPreferences.getString("host", "");
        TEMP_PORT = sharedPreferences.getInt("port", 5000);

        if (TextUtils.isEmpty(APPID)) {
            APPID = context.getString(R.string.appid);
        }
        if (TextUtils.isEmpty(TEMP_HOST)) {
            APPID = context.getString(R.string.host);
        }
    }


    private boolean checkUserInfo(CosXmlResultListener resultListener) {

        if (shouldInputUserInfo()) {
            resultListener.onFail(null, new CosXmlClientException(context.getString(R.string.please_config_paras_in_menu)), null);
            return false;
        }
